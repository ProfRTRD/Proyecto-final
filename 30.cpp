...
std::string Decodyfier(std::string &str);
std::string getter(std::string &st);

int main() {
  std::ifstream myfile;
  std::ofstream fout("Decoded.txt"); // Crear el documento en que vamos a añadir el texto modificado
  myfile.open("Coded.txt"); // Abrimos el documento que vamos a leer.
  ...
  if (myfile.is_open()) { // verificamos que el documento esté abierto
    while (myfile) { // Va a leer desde el inicio hasta el fin del documento
      ...
      fout << Decodyfier(myline); // invocación de la función codyfier, y adición  de su resultado a nuestro documento donde estamos escribiendo la codificación.
    }
